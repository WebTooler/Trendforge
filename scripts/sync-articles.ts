...
type Article = { slug: string; title: string; description: string; category: string; date: string; readTime: string; content: string[]; sources: { title: string; url: string }[] };
...
for (const file of files) {
  ...
  const sourceText = body.split(/^## Sources$/m)[1] ?? '';
  const content = main.split(/\\n\\s*\\n/).map((x) => x.trim()).filter(Boolean);
  const sources = [...sourceText.matchAll(/^- \\[(.*?)\\]\\((https:\\/\\/[^)]+)\\)$/gm)].map((m) => ({ title: m[1], url: m[2] }));
...
const source = `export type Article = ${JSON.stringify({slug:'',title:'',description:'',category:'',date:'',readTime:'',content:[],sources:[]}).replace(/"/g, '')};\n`;
