...
function write(report){fs.writeFileSync(claimPath,JSON.stringify(report,null,2)+'\n');}
...
if(articleMatchState==='none'){
  write({version:18,generatedAt:new Date().toISOString(),articlePath:'',articleTitle:'',claimCount:0,editorialCount:0,verified:0,partial:0,unsupported:0,sourceUnavailable:0,averageConfidence:0,pass:false,status:'skipped',reason:'No article was generated by the current workflow run; stale repository articles are excluded.',claims:[],editorial:[]});
  console.log('No article generated by the current run; claim verification skipped safely with exit 0.');
  process.exit(0);
}
