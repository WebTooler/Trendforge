...
const BANNED_EDITORIAL_WORD_PATTERN=new RegExp(`\\b(?:${BANNED_EDITORIAL_WORDS.join('|')})\\b`,'i');
...
export function assessHumanization(text=''){
 const value=String(text);const genericPatterns=GENERIC_AI_PATTERNS.filter(r=>r.test(value));const bannedEditorialWords=[...new Set((value.match(BANNED_EDITORIAL_WORD_PATTERN)||[]).map(x=>x.toLowerCase()))];const firstPerson=FIRST_PERSON_EXPERIENCE.test(value);const audienceMeta=AUDIENCE_META.test(value);const starts=sentenceStarterSignature(value);const repeatedStarts=[...starts.entries()].filter(([,count])=>count>=3);return {genericHits:genericPatterns.length,genericPatterns:genericPatterns.map(r=>r.source),firstPersonExperience:firstPerson,audienceMeta,repeatedSentenceStarts:repeatedStarts,bannedEditorialWords,blocked:genericPatterns.length>=2||firstPerson||audienceMeta||repeatedStarts.length>0||bannedEditorialWords.length>0};
}
