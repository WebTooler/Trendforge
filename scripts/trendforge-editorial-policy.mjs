...
export function validateDraft({title='',description='',content='',category='Technology',blueprint=null}){
 ...
 const descTrim=description.trim();if(descTrim.length>=300&&!completeSentence(descTrim))errors.push('description appears truncated or incomplete');if(fillerHits>=2)errors.push('generic/filler phrasing threshold exceeded');if(shallowSections>0)errors.push(`shallow H2 explanation detected in ${shallowSections} section(s)`);if(substantiveParagraphs<4)errors.push(`article has too few substantive paragraphs (${substantiveParagraphs}; minimum 4)`);return{passed:errors.length===0,category,policyVersion:EDITORIAL_POLICY_VERSION,metrics:{words,sentences,h2,titleChars:title.trim().length,descriptionChars:description.trim().length,fillerHits,shallowSections,substantiveParagraphs},errors};
}
