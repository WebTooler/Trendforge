...
export const hardRules=[
  ...
  'Use descriptive headings when they improve readability; never add headings just to increase length. The evidence blueprint provides an H2 recommendation, not a hard ceiling.',
...
export const qualityTargets={minWords:300,preferredMinWords:700,preferredMaxWords:1100,hardMaxWords:1500,minH2:0,preferredH2:null,maxH2:null,minDescriptionChars:100,maxDescriptionChars:320,maxTitleChars:110,minTitleChars:20,maxConsecutiveDuplicateSentences:1};
...
const paragraphTopicOverlap=(a='',b='')=>{const A=sentenceTokens(a),B=sentenceTokens(b);if(!A.size||!B.size)return 0;return[...A].filter(x=>B.has(x)).length/Math.max(1,Math.min(A.size,B.size));};
...
const duplicateEntityAliases=(text='')=>{const pairs=[['UAE','United Arab Emirates'],['US','United States'],['UK','United Kingdom'],['EU','European Union']];const hits=[];for(const sentence of String(text).match(/[^.!?]+[.!?](?:\s|$)/g)||[]){for(const [short,long] of pairs){if(new RegExp('\\b'+short+'\\b','i').test(sentence)&&new RegExp('\\b'+long+'\\b','i').test(sentence)&&!new RegExp('(?:'+long+'\\s*\\('+short+'\\)|'+short+'\\s*\\('+long+'\\))','i').test(sentence))hits.push({short,long,sentence:sentence.trim()});}}return hits;};
...
export function validateDraft({title='',description='',content='',category='Technology',blueprint=null,maxFactualClaims=0,evidenceFactMap=null}){
 const words=wordCount(content),sentences=sentenceCount(content),h2=h2Count(content); const mode=String(blueprint?.mode||'default'); const baseModeRules={rich:{minWords:650,minSentences:12,minSubstantiveParagraphs:5},bounded:{minWords:425,minSentences:10,minSubstantiveParagraphs:4},narrow:{minWords:220,minSentences:0,minSubstantiveParagraphs:2},default:{minWords:300,minSentences:0,minSubstantiveParagraphs:0}}[mode]||{minWords:300,minSentences:8,minSubstantiveParagraphs:3}; const blueprintMin=Number(blueprint?.targetWords?.min||baseModeRules.minWords); const modeRules={...baseModeRules,minWords:Math.max(0,blueprintMin),minSubstantiveParagraphs:mode==='narrow'?(blueprintMin>=280?2:1):baseModeRules.minSubstantiveParagraphs,maxH2:Number(blueprint?.maxH2||0)};const sentencesList=(String(content).match(/[^.!?]+[.!?]+/g)||[]).map(normalizedSentence).filter(Boolean);let duplicateSentence=false;for(let i=1;i<sentencesList.length;i++)if(sentencesList[i]===sentencesList[i-1]){duplicateSentence=true;break;}let repeatedSentence=false;let repeatedParagraph=false;const paragraphs=paragraphBlocks(content);const paragraphKeys=paragraphs.map(p=>String(p).toLowerCase().replace(/[^a-z0-9\\s]/g,'').replace(/\\s+/g,' ').trim()).filter(p=>p.length>=80);const paragraphKeyCounts=new Map();for(const key of paragraphKeys)paragraphKeyCounts.set(key,(paragraphKeyCounts.get(key)||0)+1);if([...paragraphKeyCounts.values()].some(n=>n>1))repeatedParagraph=true;if(!repeatedParagraph){for(let i=0;i<paragraphs.length&&!repeatedParagraph;i++){const a=paragraphs[i].trim();if(a.length<80)continue;for(let j=i+1;j<paragraphs.length;j++){if(a===paragraphs[j].trim()){repeatedParagraph=true;break;}}}}for(let i=0;i<paragraphs.length&&!repeatedParagraph;i++)for(let j=i+1;j<paragraphs.length;j++){const a=paragraphs[i],b=paragraphs[j];if(a.length<80||b.length<80)continue;const na=normalizedSentence(a),nb=normalizedSentence(b);if(na===nb||paragraphTopicOverlap(a,b)>=0.82){repeatedParagraph=true;break;}}for(let i=0;i<sentencesList.length&&!repeatedSentence;i++)for(let j=i+2;j<sentencesList.length;j++){const a=sentencesList[i],b=sentencesList[j];if(a.length<90||b.length<90)continue;if(a===b||sentenceSimilarity(a,b)>=0.88){repeatedSentence=true;break;}}
 const fillerPatterns=[/in today's (?:fast|ever-changing|rapidly changing) world/i,/in (?:the )?(?:ever-evolving|rapidly evolving|modern) (?:landscape|world)/i,/it is important to note that/i,/it's worth noting that/i,/in conclusion/i,/this article (?:will|has) (?:explore|explored|discuss)/i,/whether you are a (?:beginner|seasoned|casual)/i,/game[- ]changer/i,/game-changing/i,/revolutionary (?:new )?era/i,/plays? a crucial role/i,/delve into/i,/at the end of the day/i,/seamlessly/i,/unlock(?:s|ing)? (?:the )?(?:power|potential)/i];const fillerHits=fillerPatterns.filter(r=>r.test(content)).length;
 const humanization=assessHumanization(content);const duplicateAliases=duplicateEntityAliases(content);
 const depth=assessArticleDepth({content,blueprint});
 const emptyH2Blocks=(()=>{const lines=String(content).split(/\r?\n/);let count=0;for(let i=0;i<lines.length;i++){if(!/^##\s+/.test(lines[i]))continue;let j=i+1;while(j<lines.length&&!lines[j].trim())j++;if(j>=lines.length||/^##\s+/.test(lines[j]))count++;}return count;})();
 const shallowSections=0; // H2 depth is advisory; complete, substantive short sections are allowed.
 const substantiveParagraphs=paragraphBlocks(content).filter(p=>!/^##\s+/.test(p)&&(wordCount(p)>=25||(wordCount(p)>=5&&sentenceCount(p)>=1))).length;const titleOk=title.trim().length>=qualityTargets.minTitleChars&&title.trim().length<=qualityTargets.maxTitleChars;const descriptionOk=description.trim().length>=qualityTargets.minDescriptionChars&&description.trim().length<=qualityTargets.maxDescriptionChars;const structureOk=true;const lengthOk=words>=modeRules.minWords&&words<=qualityTargets.hardMaxWords;const errors=[];if(!titleOk)errors.push('title length outside policy');if(!descriptionOk)errors.push(`description length outside ${qualityTargets.minDescriptionChars}-${qualityTargets.maxDescriptionChars} characters`);if(!lengthOk)errors.push(`word count ${words} outside ${modeRules.minWords}-${qualityTargets.hardMaxWords} for ${mode} evidence mode`);if(sentences<modeRules.minSentences)errors.push(`article has too few complete sentences (${sentences}; minimum ${modeRules.minSentences} for ${mode})`);
 // H2 count is advisory. Structural quality is enforced through substantive sections, repetition, depth, and evidence coverage rather than a numeric ceiling.
 if(duplicateSentence)errors.push('consecutive duplicate sentence detected');if(repeatedSentence)errors.push('repeated factual sentence detected across article');if(repeatedParagraph)errors.push('repetitive paragraph/topic overlap detected across article');if(emptyH2Blocks>0)errors.push(`article contains ${emptyH2Blocks} empty H2 section(s)`);if(duplicateAliases.length)errors.push(`entity alias duplication detected (${duplicateAliases.map(x=>x.short+'/'+x.long).join(', ')})`);
 const evidenceClaimBudget=Number(maxFactualClaims||0);let evidenceClaimMetrics={materialSentenceCount:0,distinctFactClaimCount:0,mappedFactIds:[],unmappedMaterialSentences:0};if(evidenceClaimBudget>0){const editorialLike=/\b(?:could|may|might|would|should|what to watch|what remains|uncertain|unclear|suggests|highlights|illustrates|signals|overall|the takeaway|supplied evidence|evidence record|evidence records|this sentence|this second sentence|this repeats|restatement|documented .*statement|fact map|claim slot)\b/i;const synthesisAllowed=blueprint?.synthesis?.allowed===true;const synthesisMaxStatements=Math.max(0,Number(blueprint?.synthesis?.maxStatements||0));const synthesisMarker=/\b(?:in conclusion|in summary|to sum up|taken together|on balance|ultimately|overall|the takeaway)\b/i;let synthesisCount=0;const materialSentences=sentencesList.filter(s=>{if(s.length<35||editorialLike.test(s))return false;if(synthesisAllowed&&synthesisMarker.test(s)&&synthesisCount<synthesisMaxStatements){synthesisCount++;return false;}return true;});evidenceClaimMetrics={materialSentenceCount:materialSentences.length,...mapMaterialSentencesToFacts(materialSentences,evidenceFactMap)};const effectiveClaimCount=evidenceFactMap?evidenceClaimMetrics.distinctFactClaimCount+evidenceClaimMetrics.unmappedMaterialSentences:evidenceClaimMetrics.materialSentenceCount;
// Evidence capacity is a planning boundary, not a sentence-count quota. Claim verification is authoritative; do not reject a draft merely because it contains more sentences than available Fact IDs.
if(synthesisCount>synthesisMaxStatements)errors.push(`synthesis statement count ${synthesisCount} exceeds synthesis capacity ${synthesisMaxStatements}`);}const descTrim=description.trim();if(descTrim.length>=300&&!completeSentence(descTrim))errors.push('description appears truncated or incomplete');if(fillerHits>=2)errors.push('generic/filler phrasing threshold exceeded');// No numeric H2 word-count blocker; usefulness and evidence support are checked elsewhere.
if(substantiveParagraphs<modeRules.minSubstantiveParagraphs)errors.push(`article has too few substantive paragraphs (${substantiveParagraphs}; minimum ${modeRules.minSubstantiveParagraphs} for ${mode})`);if(humanization.blocked)errors.push(`humanization guard failed (${humanization.genericHits} generic AI pattern hits${humanization.firstPersonExperience?', first-person experience':''}${humanization.audienceMeta?', audience meta':''}${humanization.repeatedSentenceStarts.length?', repeated sentence starts':''})`);if(!depth.passed)errors.push(...depth.errors);return{passed:errors.length===0,category,policyVersion:EDITORIAL_POLICY_VERSION,metrics:{words,sentences,h2,emptyH2Blocks,titleChars:title.trim().length,descriptionChars:description.trim().length,fillerHits,duplicateEntityAliases:duplicateAliases,shallowSections,substantiveParagraphs,mode,modeRules,humanization,depth,evidenceClaimBudget,evidenceClaimMetrics,claimBudgetSynthesisAllowance:blueprint?.synthesis?.allowed===true?Math.max(0,Number(blueprint?.synthesis?.maxStatements||0)):0},errors};
}

export function buildWriterContract(category='Technology'){
 const profile=categoryProfiles[category]||categoryProfiles.Technology;return [`TREND FORGE WRITER ENGINE — editorial policy v${EDITORIAL_POLICY_VERSION}`,`CATEGORY: ${category}`,`CATEGORY PROFILE: ${profile}`,'','NON-NEGOTIABLE EDITORIAL RULES:',...hardRules.map((rule,i)=>`${i+1}. ${rule}`),'','SEMANTIC WRITING FREEDOM: Write like a professional newsroom writer, not a source copier. A sentence passes the evidence boundary when its meaning is supported by the evidence pack, even if its wording, order, grammar, emphasis, or framing differs from the source. You may paraphrase, contextualize, and synthesize multiple evidence passages. You may not introduce a new factual premise.','','QUALITY TARGETS: 700-1100 words when evidence supports it; never pad. Use as many useful H2 sections as the story naturally requires; H2 count is advisory only.','LENGTH POLICY: Minimum article length is 300 words for the default evidence mode; richer evidence modes may use higher evidence-relative targets. Never pad with unsupported material. A shorter article is valid only when its mode permits it and all downstream gates pass.','STRUCTURE: strong original headline → useful opening → evidence/context → implications or steps → limitations/uncertainty → practical takeaway.','FACT DISCIPLINE: every material factual claim must be traceable in meaning to supplied research or verified source evidence. Exact wording is not required.','ORIGINALITY: synthesize; do not imitate source wording, headline, structure, or paragraph order.','STYLE: clear, human, specific, restrained, informative.','','OUTPUT: Return ONLY one valid JSON object with exactly three string keys: title, description, content. No markdown fences and no commentary.'].join('\n');
}
