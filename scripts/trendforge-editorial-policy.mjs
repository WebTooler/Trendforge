...
export function validateDraft({title='',description='',content='',category='Technology',blueprint=null,maxFactualClaims=0,evidenceFactMap=null}){
 ...
 const h2Blocks=String(content).split(/(?=^##\\s+)/m).filter(block=>/^##\\s+/.test(block.trim()));
 const emptyH2Blocks=h2Blocks.filter(block=>{
   const lines=block.trim().split(/\\n/).slice(1).map(x=>x.trim()).filter(Boolean);
   return lines.length===0||lines.every(x=>/^##\\s+/.test(x));
 }).length;
