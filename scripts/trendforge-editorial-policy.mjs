...
export function validateDraft({title='',description='',content='',category='Technology',blueprint=null}){
 ...
 const substantiveParagraphs=paragraphBlocks(content).filter(p=>!/^##\s+/.test(p)&&wordCount(p)>=25).length;const titleOk=title.trim().length>=qualityTargets.minTitleChars&&title.trim().length<=qualityTargets.maxTitleChars;const descriptionOk=description.trim().length>=qualityTargets.minDescriptionChars&&description.trim().length<=qualityTargets.maxDescriptionChars;const structureOk=true;const lengthOk=words>=qualityTargets.minWords&&words<=qualityTargets.hardMaxWords;const errors=[];if(!titleOk)errors.push('title length outside policy');if(!descriptionOk)errors.push(`description length outside ${qualityTargets.minDescriptionChars}-${qualityTargets.maxDescriptionChars} characters`);if(!lengthOk)errors.push(`word count ${words} outside ${qualityTargets.minWords}-${qualityTargets.hardMaxWords}`);if(sentences<10)errors.push('article has too few complete sentences');
