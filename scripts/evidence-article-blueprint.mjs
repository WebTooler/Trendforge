...
export function deriveEvidenceArticleBlueprint(coverage={}) {
  ...
  if (band==='thin') {
    ...
    const minimumWords=Math.min(300,Math.max(220,Math.floor(capacityWords*0.62)));
    const maximumWords=Math.max(minimumWords,capacityWords);
    return {version:3,mode:'narrow',targetWords:{min:minimumWords,max:maximumWords,soft:Math.round((minimumWords+maximumWords)/2)},h2Guidance:{preferredMin:1,preferredMax:2,writerDecides:true,noPadding:true},maxH2:2,requireCrossCheck:false,allowContextSection:false,sectionPlan:['development','supported detail'],evidenceCapacity,instructions:['Stay narrow and factual.','Use only core story facts first; context facts cannot compensate for missing core facts.','Do not infer missing context.','Do not exceed the supported factual claim budget.'],claimBudget:{maxFactualClaims:claimCapacity}};
  }
