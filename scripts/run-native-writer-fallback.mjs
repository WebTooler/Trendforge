...
const withTimeout = async (promise, timeoutMs) => {
  let timer;
  try { return await Promise.race([promise, new Promise(resolve => { timer=setTimeout(()=>resolve({ok:false,reason:`native writer candidate budget exceeded (${timeoutMs}ms)`}),timeoutMs); })]); }
...
for(const candidate of ranked.slice(0,MAX_CANDIDATES)){
  ...
  const result=await withTimeout(generateNativeArticle({candidate,existingTitles}),Math.min(CANDIDATE_BUDGET_MS,TOTAL_BUDGET_MS-elapsed));
  if(!result.ok){console.log(`Native fallback skipped: ${candidate.category} — ${candidate.title} — ${result.reason}`);continue;}
