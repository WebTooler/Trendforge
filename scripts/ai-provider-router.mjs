...
const markSuccess=(name,meta={})=>{if(!providerConfig[name])return;const p=state.providers[name]??={failures:0,successes:0,quotaBlockedUntil:0};record(p,{at:Date.now(),ok:true,class:'success',status:200,latencyMs:Number(meta.latencyMs)||0,usage:meta.usage||null});p.lastLatencyMs=Number(meta.latencyMs)||0;p.lastUsage=meta.usage||null;p.successes=(p.successes||0)+1;p.lastSuccessAt=new Date().toISOString();p.lastStatus=200;p.lastError=null;p.lastClass='success';p.quotaBlockedUntil=0;persist();};
