...
let state={version:9,providers:{},rotationCursor:0,updatedAt:new Date().toISOString()};
...
const classify=(status,message='')=>{
  ...
  // Daily/token-per-day exhaustion is quota exhaustion, even when the provider returns HTTP 429.
  if(/free-models-per-day|daily quota|daily limit|quota_exceeded|exceeded your current quota|monthly quota|credit_balance_exhausted|insufficient_quota|no credits remaining|payment_required|tokens per day|tpd|requests per day|rpd/.test(s))return 'quota';
  ...
  // Network/timeout failures are temporary infrastructure failures and should be recoverable.
  if(/timeout|timed out|operation was aborted|network|fetch failed|econnreset|enotfound|socket|deadline exceeded|service unavailable|temporarily unavailable|empty model response|invalid structured output|topic drift/.test(s))return 'transient';
  return 'error';
};

const persist=()=>{state.version=9;state.updatedAt=new Date().toISOString();fs.mkdirSync('data',{recursive:true});fs.writeFileSync(statePath,JSON.stringify(state,null,2)+'\n');};
...
const healthRank=[...healthy].sort((a,b)=>{const pa=state.providers[a]||{},pb=state.providers[b]||{};const healthA=(pa.failures||0)*2-(pa.successes||0)*0.25;const healthB=(pb.failures||0)*2-(pb.successes||0)*0.25;if(healthA!==healthB)return healthA-healthB;return a.localeCompare(b);});
