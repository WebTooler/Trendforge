...
let state={version:6,providers:{},rotationCursor:0,updatedAt:new Date().toISOString()};
...
const classify=(status,message='')=>{
  ...
  if(/insufficient_quota|credit_balance_exhausted|no credits remaining|daily quota|quota_exceeded|exceeded your current quota|quota exceeded|monthly quota|payment_required/.test(s))return 'quota';
...
const persist=()=>{state.version=6;state.updatedAt=new Date().toISOString();fs.mkdirSync('data',{recursive:true});fs.writeFileSync(statePath,JSON.stringify(state,null,2)+'\n');};
