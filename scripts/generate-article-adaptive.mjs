...
// on candidates whose research is not actually multi-source ready. Native Writer
// remains the bounded evidence-recovery fallback for the wider queue.
const evidenceReady=item=>Boolean(item.evidenceReady)||((item.relevantReachableSourceCount??0)>=2&&(item.independentDomainCount??item.uniqueDomainCount??0)>=2);
const readyPrimary=primary.filter(evidenceReady);
const readyFallback=fallback.filter(evidenceReady);
const nonReadyPrimary=primary.filter(item=>!evidenceReady(item));
const queue=[...readyPrimary,...readyFallback,...nonReadyPrimary].slice(0,8);

console.log(`Adaptive publishing queue: ${queue.length} candidate(s) (${readyPrimary.length+readyFallback.length} evidence-ready before cap).`);
...
for(const fallbackArticle of evergreenFallbacks){
  ...
  fs.mkdirSync(articlesDir,{recursive:true});fs.writeFileSync(`${articlesDir}/${slug}.md`,markdown);console.log(`Evergreen fallback published without AI provider: ${slug}`);published=true;break;
}
if(!published)console.log('Adaptive queue and evergreen fallback exhausted without a publishable article. Pipeline continues without publishing low-quality content.');
