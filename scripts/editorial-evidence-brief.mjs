const STOP=new Set('about after again also been being could from have into more most over said some than that their there these they this what when which with will would your technology tech digital latest news article articles story stories report reports reported according development developments company companies industry today yesterday tomorrow while where whose through before between under using used uses make makes made less then still already now just even only often usually including another around really very much many somewhat generally'.split(' '));
...
export function buildEditorialEvidenceBrief({candidate={},sources=[]}={}){
  ...
  for(let si=0;si<sources.length;si++){
    ...
    for(const u of units.sort((a,b)=>b.relevance-a.relevance)){
      ...
      if(dedup.some(x=>phraseOverlap(x.text,u.text)>=Math.min(4,Math.floor(Math.min(x.text.length,u.text.length)/80))))continue;
