...
async function googleRecovery(title){
  try{
    const q=encodeURIComponent(title);
    const r=await fetch(`https://news.google.com/rss/search?q=${q}&hl=en-US&gl=US&ceid=US:en`,{signal:AbortSignal.timeout(TIMEOUT),headers:{'user-agent':'TrendForge-evidence-integrity/1.2'}});
    if(!r.ok)return[];
    ...
    for(const item of items){
      ...
      if(itemOverlap<MIN_TOPIC_OVERLAP)continue;
      for(const l of extractLinks(rawDescription)){
        const d=host(l.url); if(!d||MIRRORS.has(d)||blockedHosts.has(d))continue;
        const score=itemOverlap+overlap(title,l.text);
        candidates.push({url:l.url,text:l.text,discoveryTitle:itemTitle,discoveryDescription:description,score});
      }
      const link=clean((item.match(/<link>([\s\S]*?)<\/link>/i)||[,''])[1]);
      if(validUrl(link))candidates.push({url:link,text:itemTitle,discoveryTitle:itemTitle,discoveryDescription:description,score:itemOverlap});
    }
    const seen=new Set();
...
fs.writeFileSync(scoredPath,JSON.stringify(scored,null,2)+'\n');fs.mkdirSync('data',{recursive:true});fs.writeFileSync(outputPath,JSON.stringify({version:3,generatedAt:new Date().toISOString(),minimumIndependentSources:2,minimumIndependentDomains:2,report},null,2)+'\n');
console.log(`Evidence Integrity Preflight v3: ${report.filter(x=>x.status==='pass').length} eligible candidate(s) passed, ${blocked} blocked before AI.`);
