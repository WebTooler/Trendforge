...
const decode=(s='')=>String(s).replace(/<!\[CDATA\[/gi,' ').replace(/\]\]>/gi,' ').replace(/&amp;/gi,'&').replace(/&quot;/gi,'"').replace(/&#39;|&apos;|&#x27;/gi,"'").replace(/&lt;/gi,'<').replace(/&gt;/gi,'>').replace(/&nbsp;|&#160;/gi,' ');
...
async function fetchPage(url){try{const r=await fetch(url,{redirect:'follow',signal:AbortSignal.timeout(TIMEOUT),headers:{'user-agent':'Mozilla/5.0 (compatible; TrendForge-evidence-integrity/1.5)','accept':'text/html,application/xhtml+xml;q=0.9,*/*;q=0.8','accept-language':'en-US,en;q=0.9'}});if(!r.ok||!validUrl(r.url||url))return null;const html=await r.text();const title=clean((html.match(/<title[^>]*>([\s\S]*?)<\/title>/i)||[,''])[1]);const h1=clean((html.match(/<h1[^>]*>([\s\S]*?)<\/h1>/i)||[,''])[1]);const desc=clean((html.match(/<meta[^>]+(?:name|property)=["'](?:description|og:description)["'][^>]+content=["']([\s\S]*?)["']/i)||[,''])[1]);const canonical=clean((html.match(/<link[^>]+rel=["']canonical["'][^>]+href=["']([^"']+)["']/i)||[,''])[1]);const paragraphs=[...html.matchAll(/<p\b[^>]*>([\s\S]*?)<\/p>/gi)].map(m=>clean(m[1])).filter(x=>x.length>=45&&x.length<=3000).slice(0,120);const body=paragraphs.join(' ');return{url:r.url,title,h1,description:desc,canonical,body,bodyChars:body.length};}catch{return null;}}
function extractLinks(raw=''){const out=[];const decoded=decode(raw);for(const m of String(decoded).matchAll(/<a\b[^>]*href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/gi)){try{const u=new URL(decode(m[1]));if(u.protocol!=='https:')continue;const h=host(u.href);if(!h||MIRRORS.has(h)||blockedHosts.has(h)||looksLikeHomepage(u.href)||looksLikeFeed(u.href))continue;out.push({url:u.href,text:clean(m[2])});}catch{}}return out;}

async function recoverFromNewsFeed(title, feedUrl, resolvedFrom){
  try{
    const r=await fetch(feedUrl,{signal:AbortSignal.timeout(TIMEOUT),headers:{'user-agent':'TrendForge-evidence-integrity/1.5','accept':'application/rss+xml,application/xml,text/xml;q=0.9,*/*;q=0.8'}});
    if(!r.ok)return[];
    ...
    for(const item of items){
      ...
      for(const l of extractLinks(rawDescription))candidates.push({url:l.url,discoveryTitle:itemTitle,discoveryDescription:description,score:itemOverlap+overlap(title,l.text),resolvedFrom:`${resolvedFrom}-description-link`});
...
async function newsRecovery(title){
  ...
  for(const q of queries){
    ...
    for(const [url,kind] of feeds){
      const candidates=await recoverFromNewsFeed(title,url,kind); all.push(...candidates); if(all.length>=10)break;
    }
