...
const providerTimeout=provider=>({OpenRouter:30000,Cohere:25000,Groq:20000,Gemini:20000}[provider]||20000);
...
const commonSystem='You are the TrendForge Writer Engine. Follow the editorial policy in the prompt. Use only supplied research. Produce original, factual, useful journalism. Never fabricate facts. Return ONLY a JSON object with exactly these string fields: title, description, content.';
const jsonSchema={type:'json_schema',json_schema:{name:'trendforge_article',strict:true,schema:{type:'object',properties:{title:{type:'string'},description:{type:'string'},content:{type:'string'}},required:['title','description','content'],additionalProperties:false}}};

const request=async(provider,prompt)=>{
  for(let attempt=0;attempt<=MAX_TRANSIENT_RETRIES;attempt++){
    const signal=AbortSignal.timeout(providerTimeout(provider));let r;
    if(provider==='Groq'){
      r=await fetch('https://api.groq.com/openai/v1/chat/completions',{method:'POST',signal,headers:{Authorization:`Bearer ${process.env.GROQ_API_KEY}`,'Content-Type':'application/json'},body:JSON.stringify({model:providerModel(provider),max_completion_tokens:2800,reasoning_effort:'low',response_format:{type:'json_object'},messages:[{role:'system',content:commonSystem},{role:'user',content:prompt}]})});
...
const topicTokens=(text='')=>new Set(String(text).toLowerCase().replace(/[^a-z0-9]+/g,' ').split(/\s+/).filter(w=>w.length>=4&&!new Set(['about','after','again','also','been','being','could','from','have','into','more','most','over','said','some','than','that','their','there','these','they','this','what','when','which','with','will','would','your','technology','tech','digital','latest','news','update','today','company','industry','development','developments']).has(w)));
