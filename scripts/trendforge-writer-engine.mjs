...
const retryDelay=(response,attempt)=>{
  ...
  return Math.min(500*(2**attempt)+Math.floor(Math.random()*250),4000);
};
...
const request=async(provider,prompt)=>{
  for(let attempt=0;attempt<=MAX_TRANSIENT_RETRIES;attempt++){
    ...
    if(provider==='Groq'){
      ...
      r=await fetch('https://api.openai.com/v1/responses',{method:'POST',signal,headers:{Authorization:`Bearer ${process.env.OPENAI_API_KEY}`,'Content-Type':'application/json'},body:JSON.stringify({model:process.env.OPENAI_MODEL||'gpt-5.6-luna',input:prompt,max_output_tokens:2200})});
    }
    if(r.ok){
      if(provider==='Groq'){const j=await r.json();return j.choices?.[0]?.message?.content||'';}
      if(provider==='Gemini'){const j=await r.json();return j.candidates?.[0]?.content?.parts?.map(p=>p.text||'').join('')||'';}
      const j=await r.json();return j.output_text||j.output?.flatMap(x=>x.content||[]).filter(x=>x.type==='output_text'&&x.text).map(x=>x.text).join('')||'';
    }
    const body=(await r.text()).slice(0,1000);
    const retryable=r.status===429||r.status>=500;
    if(retryable&&attempt<MAX_TRANSIENT_RETRIES){await sleep(retryDelay(r,attempt));continue;}
...
const parseWriterJson=(text='')=>{
  ...
  for(const candidate of [cleaned,cleaned.replace(/^\s*json\s*/i,''),(()=>{const start=cleaned.indexOf('{'),end=cleaned.lastIndexOf('}');return start>=0&&end>start?cleaned.slice(start,end+1):'';})()]){
...
export async function generateWithTrendForgeWriter({prompt,category='Technology'}){
  ...
  for(const provider of available){
    ...
    try{
      ...
      return{text:JSON.stringify(draft),provider,policyVersion:'1.3'};
