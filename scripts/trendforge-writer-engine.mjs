...
const request=async(provider,prompt)=>{
  for(let attempt=0;attempt<=MAX_TRANSIENT_RETRIES;attempt++){
    ...
    if(provider==='Groq')r=await fetch('https://api.groq.com/openai/v1/chat/completions',{method:'POST',signal,headers:{Authorization:`Bearer ${process.env.GROQ_API_KEY}`,'Content-Type':'application/json'},body:JSON.stringify({model:providerModel(provider),max_completion_tokens:2400,reasoning_effort:'low',response_format:groqFormat,messages:[{role:'system',content:commonSystem},{role:'user',content:prompt}]})});
    else if(provider==='Gemini')r=await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${providerModel(provider)}:generateContent`,{method:'POST',signal,headers:{'x-goog-api-key':process.env.GEMINI_API_KEY,'Content-Type':'application/json'},body:JSON.stringify({contents:[{parts:[{text:`${commonSystem}\n\n${prompt}`}]}],generationConfig:{responseMimeType:'application/json',maxOutputTokens:2800}})});
...
const entityTokens=(text='')=>{const known=['openai','anthropic','google','microsoft','meta','amazon','apple','nvidia','tesla','xai','mistral','deepmind','bitcoin','ethereum','xrp','solana','clarity act','fed','senate','india','indian','consumer regulator'];const lower=String(text).toLowerCase();return new Set(known.filter(x=>lower.includes(x)));};
...
const topicAlignment=(requested,draft)=>{const a=topicTokens(requested),b=topicTokens(`${draft.title} ${draft.description} ${draft.content.slice(0,7000)}`),titleB=topicTokens(draft.title);const shared=[...a].filter(x=>b.has(x)),titleShared=[...a].filter(x=>titleB.has(x));const entities=[...entityTokens(requested)].filter(x=>entityTokens(`${draft.title} ${draft.description} ${draft.content.slice(0,7000)}`).has(x));return{score:shared.length,shared:shared.slice(0,12),titleShared:titleShared.slice(0,12),entities:entities.slice(0,8),passed:titleShared.length>=2||shared.length>=5||entities.length>=1};};
...
export async function generateWithTrendForgeWriter({prompt,category='Technology',expectedTitle=''}){
  ...
  for(const provider of available){
    ...
    try{
      ...
      if(!text.trim()){mark(provider,200,'Empty model response');console.log(`TrendForge Writer Engine: ${provider} returned an empty response after ${Date.now()-started}ms; trying next provider.`);continue;}
      const draft=parseWriterJson(text);
      if(!draft||typeof draft.title!=='string'||typeof draft.description!=='string'||typeof draft.content!=='string'){mark(provider,200,'Invalid structured output');console.log(`TrendForge Writer Engine: ${provider} returned invalid structured output after ${Date.now()-started}ms; trying next provider.`);continue;}
      const alignment=topicAlignment(requested,draft);
      if(requested&&!alignment.passed){console.log(`TrendForge Writer Engine: ${provider} output rejected for topic drift after ${Date.now()-started}ms — requested topic overlap ${alignment.score}; title overlap ${alignment.titleShared.join(', ')||'none'}; entity overlap ${alignment.entities.join(', ')||'none'}.`);continue;}
