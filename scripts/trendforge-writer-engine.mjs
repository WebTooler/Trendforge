...
const request=async(provider,prompt)=>{
  ...
  const r=await fetch('https://api.openai.com/v1/responses',{method:'POST',signal,headers:{Authorization:`Bearer ${process.env.OPENAI_API_KEY}`,'Content-Type':'application/json'},body:JSON.stringify({model:process.env.OPENAI_MODEL||'gpt-5.6-luna',input:prompt,max_output_tokens:2600})});
...
export async function generateWithTrendForgeWriter({prompt,category='Technology'}){
  ...
  for(const provider of available){
    ...
    try{
      ...
      if(!draft||typeof draft.title!=='string'||typeof draft.description!=='string'||typeof draft.content!=='string'){console.log(`TrendForge Writer Engine: ${provider} returned invalid structured output; trying next provider.`);continue;}
